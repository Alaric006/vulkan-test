...
private:
	...
	void pickPhysicalDevice() {
		...
		std::vector<uint32_t> scores(deviceCount);
		for (uint32_t i = 0; i < deviceCount; i++) {
			scores[i] = rateDevice(devices[i]);
		}
		...
		physicalDevice = devices[maxScoreIndex];
	...
	void updateUniformBuffer(uint32_t currentImage) {
		...
		ubo.view = glm::lookAt(position, position + direction, glm::cross(right, direction));
